...
export function generateStaticParams() {
  const slugs = getChapterSlugs('junior');
  return slugs.map((slug) => ({ slug }));
}

export default function Page({ params }: { params: Promise<{ slug: string }> }) {
  return <ChapterPageWrapper level="junior" paramsPromise={params} />;
}

async function ChapterPageWrapper({ level, paramsPromise }: { level: 'junior'; paramsPromise: Promise<{ slug: string }> }) {
